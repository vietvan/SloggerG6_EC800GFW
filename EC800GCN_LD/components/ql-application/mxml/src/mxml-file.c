...
static int user_getc(QFILE *fd)
{
    return -1;
 }

static int user_putc(int ch,QFILE *fd)
{
    return -1;
 }
...
mxml_fd_read(_mxml_fdbuf_t *buf)		/* I - File descriptor buffer */
{
  ...
  */
   ...
   //  ql_fread((void*)(buf->buffer),sizeof(buf->buffer),1,buf->fd)
  while ((bytes = (int)ql_fread((void*)(buf->buffer),sizeof(buf->buffer),1,buf->fd)) < 0)
#ifdef EINTR
...
mxml_fd_write(_mxml_fdbuf_t *buf)	/* I - File descriptor buffer */
{
  ...
  */
     // ql_fwrite((void *)ptr, buf->current - ptr, 1, buf->fd)

  for (ptr = buf->buffer; ptr < buf->current; ptr += bytes)
    if ((bytes = (int)ql_fwrite((void *)ptr, buf->current - ptr, 1, buf->fd)) < 0)
      return (-1);
	...
	*/
	...
	else if ((ch & 0xe0) == 0xc0)
	{
	  ...
	  */
	  ...
	  if ((temp = user_getc(fp)) == EOF || (temp & 0xc0) != 0x80)
	    return (EOF);
	...
	*/
        ...
        else if (ch >= 0xd800 && ch <= 0xdbff)
	{
	  ...
	  */
          ...
          int lch = user_getc(fp);
          lch = (lch << 8) | user_getc(fp);
...
{
	return (user_putc(ch, (QFILE *)p) == EOF ? -1 : 0);
}
