...
int32_t core_http_setopt(void *handle, core_http_option_t option, void *data)
{
    ...
    switch (option) {
		...
		break;
		case CORE_HTTPOPT_PROFILE_IDX: {
            http_handle->profile_idx = *(uint16_t *)data;
        }
        break;
...
static int32_t _core_http_recv_body(core_http_handle_t *http_handle, uint32_t body_total_len)
{
    ...
    if (res > 0) {
		...
		//	uart_printf("func:%s  line:%d packet.data.body.len: %d  \r\n",__func__,__LINE__,packet.data.body.len);
		//	uart_printf("func:%s  line:%d packet.data.body.buffer: %s  \r\n",__func__,__LINE__,packet.data.body.buffer);
            http_handle->core_recv_handler(http_handle, &packet, http_handle->core_userdata);
        }
    }
	//uart_printf("func:%s  line:%d buffer: %s  \r\n",__func__,__LINE__,buffer);
	if(buffer)
